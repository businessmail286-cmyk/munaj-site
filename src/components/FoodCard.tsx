...
export const FoodCard: React.FC<FoodCardProps> = ({ food, onOpenDetails }) => {
  ...
  return (
    <div
      ...
      className="group bg-white rounded-2xl border border-neutral-200/80 hover:border-amber-400/80 shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden cursor-pointer relative"
    >
      {/* Image Wrap */}
      <div className="relative aspect-4/3 w-full bg-neutral-100 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
          {isFeatured && (
            <span className="bg-amber-500 text-white text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full shadow-xs flex items-center gap-1">
              <Flame className="w-3 h-3 fill-white" /> Special
            </span>
          )}
          {food.category_name && (
            <span className="bg-neutral-900/80 backdrop-blur-xs text-neutral-200 text-[10px] font-semibold px-2.5 py-0.5 rounded-full">
              {food.category_name}
            </span>
          )}
        </div>
        ...
        <div className="absolute inset-0 bg-neutral-950/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
          <span className="bg-white/95 backdrop-blur-xs text-neutral-900 text-xs font-bold px-3 py-1.5 rounded-full shadow-md flex items-center gap-1.5 transform translate-y-2 group-hover:translate-y-0 transition-transform">
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1 justify-between">
        <div>
          <h3 className="font-bold text-neutral-900 text-base group-hover:text-amber-600 transition-colors line-clamp-1 leading-snug">
            {food.name}
          </h3>
